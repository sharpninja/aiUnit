...
- TEST-AIUNIT-066: Local validation runs dotnet restore, Release dotnet test, CI-versioned dotnet pack to a temporary output, and git diff --check for the Azure pipeline command path; NuGet push requires a NuGetApiKey secret in Azure Pipelines.
